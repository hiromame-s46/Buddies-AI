...
async function send(text,useSkills=false,alreadyAdded=false){
  ...
  try{const response=await fetch('lfm-api.php',{method:'POST',cache:'no-store',signal:controller.signal,headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(payload(text,useSkills))});const raw=await response.text(),contentType=response.headers.get('content-type')||'';let data;try{data=JSON.parse(raw)}catch{const detail=raw.trim().replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').slice(0,100);throw new Error(response.status>=500?'サーバーで回答APIを実行できませんでした。時間を空けて再度お試しください。':detail?`APIエラー: ${detail}`:`通信エラー（HTTP ${response.status}）`)}if(!contentType.includes('application/json')&&!data)throw new Error('回答APIの形式が正しくありません。');if(!response.ok||!data.ok)throw new Error(errorMessage(response.status,data));loading.row.remove();add('assistant',data.text);renderSearchResults(data.results)}catch(error){loading.row.remove();if(error.name==='AbortError')toast('生成を中止しました');else add('assistant',error.message)}finally{generating=false;controller=null;setSendMode(false);resize();$('input').focus()}
}
